...
namespace InvoiceManager
{
    public partial class ClientDetails : Form
    {
        ...
        {
            string queryString = "SELECT zip_code, city, street, building_number, account, bank, bank_city FROM client WHERE id_client = @id";

            SqlCommand command = new SqlCommand(queryString, dataBase.getConnection());
            command.Parameters.AddWithValue("@id", clientId); // Параметр, щоб уникнути SQL-ін'єкцій
        ...
        {
            string queryString = "UPDATE [invoiceManager].[dbo].[client] " +
            ...
            command.Parameters.AddWithValue("@id", clientId); // Використовуємо id клієнта для оновлення конкретного запису
